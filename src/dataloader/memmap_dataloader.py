...
class Dataset(torch.utils.data.Dataset):
    ...
    def get_file_indices_from_ts_range(self, ts_range, vbl_type):
        rg = self.dataset._get_file_indices((*ts_range, 1), vbl_type, {})
...
if __name__ == "__main__":
    ...
    datapath = ["/home_tools/data/fdl/mmap/debug/debug.dill", "/home_tools/data/fdl/mmap/debug2/debug.dill"]
    ...
    # a = dataset["era140625/t_850hPa"]
    # b = dataset["simsat140625/clbt"]
    print(" reading finished...")

    # Scenario 1a: PyTorch dataloader
    ...
    # dataset.selected_partition_id = 0
    # dataloader = torch.utils.data.DataLoader(dataset, **params)
    # for sample in tqdm(dataloader):
    #     lst.extend(sample[0].numpy().tolist())
    #
    # h = (np.array(sorted(lst)) - datetime.datetime(2010, 1, 1, 0).timestamp()) // 3600
    # a = 5

    # b = dataset["era5625/t:600hPa"]

    from src.dataloader.transforms import Normalise

    trsform = Normalise(dataset, normalisation_dict={"__temp": "global",
                                                     "__const": "global",
                                                     "era5625/tp": "global"})
    ...
    for sample in tqdm(dataloader):
        # print(sample[0].keys())
        # print(sample[0]["__sample_modes__"])
        # print(sample[0]["__sample_ts__"])
        b = 5
        c = trsform(sample)
        pass
    ...
    # Scenario 2: Find linear correlations between variables
    from scipy.stats import spearmanr

    x = dataset[((datetime.datetime(2010, 1, 1, 0).timestamp(), datetime.datetime(2010, 12, 31, 0).timestamp(), 3600),
                 ["yera5625/t", "yera5625/tp"], None)]
    t = x["yera5625/t"]
    tp = x["yera5625/tp"]
    res = (t * tp).std()
