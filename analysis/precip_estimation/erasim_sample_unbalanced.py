...
if __name__ == "__main__":
            ...
            "sample":  # sample sections
                {
                    ...
                    "__const__lon2d": {"vbl": "__const__lon2d",
                                       "val": np.repeat(np.expand_dims(np.linspace(0.0, 1.0, grid_shape[1]), axis=0),
                                                        grid_shape[0], axis=0)},
                    "z_300hPa": {"vbl": "era5625/z_300hPa",
                                 "t": np.array([dlt]) * 3600,
                                 "interpolate": ["nan", "nearest_past", "nearest_future"][1]},
                    "z_500hPa": {"vbl": "era5625/z_500hPa",
                                 "t": np.array([dlt]) * 3600,
                                 "interpolate": ["nan", "nearest_past", "nearest_future"][1]},
                    "z_850hPa": {"vbl": "era5625/z_850hPa",
                                 "t": np.array([dlt]) * 3600,
                                 "interpolate": ["nan", "nearest_past", "nearest_future"][1]},
                    "t_300hPa": {"vbl": "era5625/t_300hPa",
                                 "t": np.array([dlt]) * 3600,
                                 "interpolate": ["nan", "nearest_past", "nearest_future"][1]},
                    "t_500hPa": {"vbl": "era5625/t_500hPa",
                                 "t": np.array([dlt]) * 3600,
                                 "interpolate": ["nan", "nearest_past", "nearest_future"][1]},
                    "t_850hPa": {"vbl": "era5625/t_850hPa",
                                 "t": np.array([dlt]) * 3600,
                                 "interpolate": ["nan", "nearest_past", "nearest_future"][1]},
                    "q_300hPa": {"vbl": "era5625/q_300hPa",
                                 "t": np.array([dlt]) * 3600,
                                 "interpolate": ["nan", "nearest_past", "nearest_future"][1]},
                    "q_500hPa": {"vbl": "era5625/q_500hPa",
                                 "t": np.array([dlt]) * 3600,
                                 "interpolate": ["nan", "nearest_past", "nearest_future"][1]},
                    "q_850hPa": {"vbl": "era5625/q_850hPa",
                                 "t": np.array([dlt]) * 3600,
                                 "interpolate": ["nan", "nearest_past", "nearest_future"][1]},
                    "sp": {"vbl": "era5625/sp",
                           "t": np.array([dlt]) * 3600,
                           "interpolate": ["nan", "nearest_past", "nearest_future"][1]},
                    "clwc_300hPa": {"vbl": "era5625/clwc_300hPa",
                                    "t": np.array([dlt]) * 3600,
                                    "interpolate": ["nan", "nearest_past", "nearest_future"][1]},
                    "clwc_500hPa": {"vbl": "era5625/clwc_500hPa",
                                    "t": np.array([dlt]) * 3600,
                                    "interpolate": ["nan", "nearest_past", "nearest_future"][1]},
                    "clwc_850hPa": {"vbl": "era5625/clwc_850hPa",
                                    "t": np.array([dlt]) * 3600,
                                    "interpolate": ["nan", "nearest_past", "nearest_future"][1]},
                    "ciwc_300hPa": {"vbl": "era5625/ciwc_300hPa",
                                    "t": np.array([dlt]) * 3600,
                                    "interpolate": ["nan", "nearest_past", "nearest_future"][1]},
                    "ciwc_500hPa": {"vbl": "era5625/ciwc_500hPa",
                                    "t": np.array([dlt]) * 3600,
                                    "interpolate": ["nan", "nearest_past", "nearest_future"][1]},
                    "ciwc_850hPa": {"vbl": "era5625/ciwc_850hPa",
                                    "t": np.array([dlt]) * 3600,
                                    "interpolate": ["nan", "nearest_past", "nearest_future"][1]},
                    "t2m": {"vbl": "era5625/t2m",
                            "t": np.array([dlt]) * 3600,
    ...
    def get_pixels(args):
        ...
        for j, frame_idx in enumerate(frame_idxs):
            ...
            #print("DATAIDX: {} FRAMEIDX: {}".format(data_idx, frame_idx))

            if not sum(data_idx):
                
                continue
            #frame = dataset.dataset[((dr[0], dr[1], 3600), ["imerg5625/precipitationcal"], {})][frame_idxs]
            #sample = dataset[frame_idx]
            #frame = sample[0]["label"]["tp"][0,0]
            ...
            for c in range(4):
                ...
                if data_idx[c].size == 0.0:
                    #print("NONE FOUND!")
                    continue
                #print("data_idx[c]: ", data_idx[c], idxs[0])
                try:
    ...
    with open("erasim_samples_unb_{}.json".format(part), "w") as f:
        json.dump(totres, f)
