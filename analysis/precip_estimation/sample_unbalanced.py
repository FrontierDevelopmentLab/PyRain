...
if __name__ == "__main__":
    ...
    import math
    # draw unequal number of idxs from each class
    for c in range(4):
        idx_lst = []
        print("class {} draw {} samples...", c, int(n_samples*f[c]))
            ...
            for k,v in sample_conf["mode0"]["sample"].items():
                if k[:3] == "__c":
                    ...
                    if len(g.shape) == 3:
                        ...
                        sam[0]["sample"][k] = dataset.dataset[((*partition_conf[part]["timerange"], 3600), [v["vbl"]], {})][frame_idx]
            for k,v in sample_conf["mode0"]["label"].items():
                sam[0]["label"][k] = dataset.dataset[((*partition_conf[part]["timerange"], 3600), [v["vbl"]], {})][frame_idx]
    ...
    with open("era_samples_unb_{}_1M.json".format(part), "w") as f:
        json.dump(totres, f)
